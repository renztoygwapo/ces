
	<!-- BEGIN PAGE HEADER-->
			
				<!-- BEGIN PAGE HEADER-->
			<h3 class="page-title">
			Student Reports <small></small>
			</h3>
			<div class="page-bar">
				<ul class="page-breadcrumb">
					<li>
						<i class="fa fa-home"></i>
						<a href="index.html">Home</a>
						<i class="fa fa-angle-right"></i>
					</li>
					<li>
						<a href="#">Student Reports</a>
					</li>
				</ul>
		
			</div>
			<!-- END PAGE HEADER-->


			<!-- header unta neh do -->
			<div class="portlet box blue">
						<div class="portlet-title">
							<div class="caption">
								<i class="fa fa-edit"></i>Periodic Rating
							</div>
							<div class="tools">
								<a href="javascript:;" class="collapse">
								</a>
								<a href="#portlet-config" data-toggle="modal" class="config">
								</a>
								<a href="javascript:;" class="reload">
								</a>
								<a href="javascript:;" class="remove">
								</a>
							</div>
						</div>
						<div class="portlet-body">
							
							<div id="sample_editable_1_wrapper" class="dataTables_wrapper no-footer">
					        <div class="table-scrollable">
					        <table class="table table-striped table-hover table-bordered dataTable no-footer" id="sample_editable_1" role="grid" aria-describedby="sample_editable_1_info">
							<thead>
							<tr role="row">
								<th class="" tabindex="0" aria-controls="sample_editable_1" rowspan="1" colspan="1" aria-sort="ascending" aria-label="
									Learning Areas
								: activate to sort column ascending" style="width: 400px;">
									 <center>Learning Areas</center>
								</th>
								<th class=" " tabindex="0" aria-controls="sample_editable_1" rowspan="1" colspan="1" aria-label="
									 1
								: activate to sort column ascending" style="width: 100px;"><center>1</center>
									 
								</th>
								<th class=" " tabindex="0" aria-controls="sample_editable_1" rowspan="1" colspan="1" aria-label="
									 2
								: activate to sort column ascending" style="width: 100px;">
									<center>2</center>
								</th>
								<th class=" " tabindex="0" aria-controls="sample_editable_1" rowspan="1" colspan="1" aria-label="
									 3
								: activate to sort column ascending" style="width: 100px;">
									<center>3</center>
								</th>
								<th class=" " tabindex="0" aria-controls="sample_editable_1" rowspan="1" colspan="1" aria-label="
									 4
								: activate to sort column ascending" style="width: 100px;">
									<center>4</center>
								</th>
								<th class=" " tabindex="0" aria-controls="sample_editable_1" rowspan="1" colspan="1" aria-label="
									 Final Rating
								: activate to sort column ascending" style="width: 147px;">
									<center>Final Rating</center>
								</th>
								<th class=" " tabindex="0" aria-controls="sample_editable_1" rowspan="1" colspan="1" aria-label="
									 Edit
								: activate to sort column ascending" style="width: 91px;">
									 <center> Edit</center>
								</th>
								</tr>
							</thead>
							<tbody>
				
							<tr role="row" class="odd">
								<td class=" _1">
									 Filipino
								</td>
								<td>
									
								</td>
								<td>
									 
								</td>
								<td class="center">
									 
								</td>
								<td>
										
								</td>
								<td>
									 
								</td>
								<td>
									<a class="edit" href="javascript:;">
									Edit </a>
								</td>

							</tr>
							<tr role="row" class="odd">
								<td class=" _1">
									 English
								</td>
								<td>
									
								</td>
								<td>
									 
								</td>
								<td class="center">
									 
								</td>
								<td>
										
								</td>
								<td>
									 
								</td>
								<td>
									<a class="edit" href="javascript:;">
									Edit </a>
								</td>
								
							</tr>
							<tr role="row" class="odd">
								<td class=" _1">
									 Mathematics
								</td>
								<td>
									
								</td>
								<td>
									 
								</td>
								<td class="center">
									 
								</td>
								<td>
										
								</td>
								<td>
									 
								</td>
								<td>
									<a class="edit" href="javascript:;">
									Edit </a>
								</td>
								
							</tr>
							<tr role="row" class="odd">
								<td class=" _1">
									 Science & Health
								</td>
								<td>
								</td>
								<td>
								</td>
								<td class="center">
								</td>
								<td>
								</td>
								<td>
								</td>
								<td>
									<a class="edit" href="javascript:;">
									Edit </a>
								</td>
								
							</tr>
							<tr role="row" class="odd">
								<td class=" _1">
									 MAKABAYAN
								</td>
								<td>
									
								</td>
								<td>
									 
								</td>
								<td class="center">
									 
								</td>
								<td>
										
								</td>
								<td>
									 
								</td>
								<td>
								</td>
							</tr>
							<tr role="row" class="odd">
								<td class=" _1">
									 *Heograpiya Kasaysayan/Sibika(HKS)
								</td>
								<td>
									
								</td>
								<td>
									 
								</td>
								<td class="center">
									 
								</td>
								<td>
										
								</td>
								<td>
									 
								</td>
								<td>
									<a class="edit" href="javascript:;">
									Edit </a>
								</td>
								
							</tr>
							<tr role="row" class="odd">
								<td class=" _1">
									 *Edukasyong Pantahanan at Pangkabuhayan (EPP)
								</td>
								<td>
									
								</td>
								<td>
									 
								</td>
								<td class="center">
									 
								</td>
								<td>
										
								</td>
								<td>
									 
								</td>
								<td>
									<a class="edit" href="javascript:;">
									Edit </a>
								</td>
							</tr>
							<tr role="row" class="odd">
								<td class=" _1">
									 *Musika, Sining at Edukasyon sa Pagpapalakas ng Katawan (MSEP)
								</td>
								<td>
									
								</td>
								<td>
									 
								</td>
								<td class="center">
									 
								</td>
								<td>
										
								</td>
								<td>
									 
								</td>
								<td>
									<a class="edit" href="javascript:;">
									Edit </a>
								</td>
								
							</tr>
							<tr role="row" class="odd">
								<td class=" _1">
									 Character Education
								</td>
								<td>
									
								</td>
								<td>
									 
								</td>
								<td class="center">
									 
								</td>
								<td>
										
								</td>
								<td>
									 
								</td>
								<td>
									<a class="edit" href="javascript:;">
									Edit </a>
								</td>
								
							</tr>
							<tr role="row" class="odd">
								<td class=" _1">
									<strong>Average</strong> 
								</td>
								<td>
									
								</td>
								<td>
									 
								</td>
								<td class="center">
									 
								</td>
								<td>
										
								</td>
								<td>
									 
								</td>
								<td>
									<a class="edit" href="javascript:;">
									Edit </a>
								</td>
								
							</tr>
							
							



							</tbody>
							</table>
							</div>
							</div>
					</div>
					</div>
					<div class="portlet box blue">
						<div class="portlet-title">
							<div class="caption">
								<i class="fa fa-edit"></i>Attendance Record
							</div>
							<div class="tools">
								<a href="javascript:;" class="collapse">
								</a>
								<a href="#portlet-config" data-toggle="modal" class="config">
								</a>
								<a href="javascript:;" class="reload">
								</a>
								<a href="javascript:;" class="remove">
								</a>
							</div>
						</div>
						<div class="portlet-body">
							
							<div id="sample_editable_1_wrapper" class="dataTables_wrapper no-footer">
					        <div class="table-scrollable">
					        <table class="table table-striped table-hover table-bordered dataTable no-footer" id="sample_editable_1" role="grid" aria-describedby="sample_editable_1_info">
							<thead>
							<tr role="row">
								<th class="" tabindex="0" aria-controls="sample_editable_1" rowspan="1" colspan="1" aria-sort="ascending" aria-label="
									Learning Areas
								: activate to sort column ascending" style="width: 1000px;">
									 <center></center>
								</th>
								<th class=" " tabindex="0" aria-controls="sample_editable_1" rowspan="1" colspan="1" aria-label="
									 1
								: activate to sort column ascending" style="width: 100px;"><center>Jun.</center>
									 
								</th>
								<th class=" " tabindex="0" aria-controls="sample_editable_1" rowspan="1" colspan="1" aria-label="
									 2
								: activate to sort column ascending" style="width: 100px;">
									<center>Jul.</center>
								</th>
								<th class=" " tabindex="0" aria-controls="sample_editable_1" rowspan="1" colspan="1" aria-label="
									 3
								: activate to sort column ascending" style="width: 100px;">
									<center>Aug.</center>
								</th>
								<th class=" " tabindex="0" aria-controls="sample_editable_1" rowspan="1" colspan="1" aria-label="
									 4
								: activate to sort column ascending" style="width: 100px;">
									<center>Sept.</center>
								</th>
								<th class=" " tabindex="0" aria-controls="sample_editable_1" rowspan="1" colspan="1" aria-label="
									 Final Rating
								: activate to sort column ascending" style="width: 147px;">
									<center>Oct.</center>
								</th>
								<th class=" " tabindex="0" aria-controls="sample_editable_1" rowspan="1" colspan="1" aria-label="
									 Final Rating
								: activate to sort column ascending" style="width: 147px;">
									<center>Nov.</center>
								</th>
								<th class=" " tabindex="0" aria-controls="sample_editable_1" rowspan="1" colspan="1" aria-label="
									 Final Rating
								: activate to sort column ascending" style="width: 147px;">
									<center>Dec.</center>
								</th>
								<th class=" " tabindex="0" aria-controls="sample_editable_1" rowspan="1" colspan="1" aria-label="
									 Final Rating
								: activate to sort column ascending" style="width: 147px;">
									<center>Jan.</center>
								</th>
								<th class=" " tabindex="0" aria-controls="sample_editable_1" rowspan="1" colspan="1" aria-label="
									 Final Rating
								: activate to sort column ascending" style="width: 147px;">
									<center>Feb.</center>
								</th>
								<th class=" " tabindex="0" aria-controls="sample_editable_1" rowspan="1" colspan="1" aria-label="
									 Final Rating
								: activate to sort column ascending" style="width: 147px;">
									<center>Mar.</center>
								</th>
								<th class=" " tabindex="0" aria-controls="sample_editable_1" rowspan="1" colspan="1" aria-label="
									 Final Rating
								: activate to sort column ascending" style="width: 147px;">
									<center>Apr.</center>
								</th>
								<th class=" " tabindex="0" aria-controls="sample_editable_1" rowspan="1" colspan="1" aria-label="
									 Final Rating
								: activate to sort column ascending" style="width: 147px;">
									<center>Total</center>
								</th>
								<th class=" " tabindex="0" aria-controls="sample_editable_1" rowspan="1" colspan="1" aria-label="
									 Edit
								: activate to sort column ascending" style="width: 91px;">
									 <center> Edit</center>
								</th>
								</tr>
							</thead>
							<tbody>
				
						
						
							<tr role="row" class="odd">
								<td class=" _1">
									No. of School Days
								</td>
								<td>
									
								</td>
								<td>
									 
								</td>
								<td>
									 
								</td>
								<td>
										
								</td>
								<td>
									 
								</td>
								<td>
									 
								</td>
								<td>
									 
								</td>
								<td>
									 
								</td>
								<td>
									 
								</td>
								<td>
									 
								</td>
								<td>
									 
								</td>
								<td>
									 
								</td>

								<td>
									<a class="edit" href="javascript:;">
									Edit </a>
								</td>
							</tr>
				<tr role="row" class="odd">
								<td class=" _1">
									No. of School Days
								</td>
								<td>
									
								</td>
								<td>
									 
								</td>
								<td>
									 
								</td>
								<td>
										
								</td>
								<td>
									 
								</td>
								<td>
									 
								</td>
								<td>
									 
								</td>
								<td>
									 
								</td>
								<td>
									 
								</td>
								<td>
									 
								</td>
								<td>
									 
								</td>
								<td>
									 
								</td>

								<td>
									<a class="edit" href="javascript:;">
									Edit </a>
								</td>
							</tr>
							<tr role="row" class="odd">
								<td class=" _1">
									No. of School Days Absent
								</td>
								<td>
									
								</td>
								<td>
									 
								</td>
								<td>
									 
								</td>
								<td>
										
								</td>
								<td>
									 
								</td>
								<td>
									 
								</td>
								<td>
									 
								</td>
								<td>
									 
								</td>
								<td>
									 
								</td>
								<td>
									 
								</td>
								<td>
									 
								</td>
								<td>
									 
								</td>

								<td>
									<a class="edit" href="javascript:;">
									Edit </a>
								</td>
							</tr>
							<tr role="row" class="odd">
								<td class=" _1">
									No. of Times Tardy
								</td>
								<td>
									
								</td>
								<td>
									 
								</td>
								<td>
									 
								</td>
								<td>
										
								</td>
								<td>
									 
								</td>
								<td>
									 
								</td>
								<td>
									 
								</td>
								<td>
									 
								</td>
								<td>
									 
								</td>
								<td>
									 
								</td>
								<td>
									 
								</td>
								<td>
									 
								</td>

								<td>
									<a class="edit" href="javascript:;">
									Edit </a>
								</td>
							</tr>
				
				
				
							</tbody>
							</table>
							</div>
							</div>
					</div>
					</div>
					<div class="portlet box blue">
						<div class="portlet-title">
							<div class="caption">
								<i class="fa fa-edit"></i>Attendance Record
							</div>
							<div class="tools">
								<a href="javascript:;" class="collapse">
								</a>
								<a href="#portlet-config" data-toggle="modal" class="config">
								</a>
								<a href="javascript:;" class="reload">
								</a>
								<a href="javascript:;" class="remove">
								</a>
							</div>
						</div>
						<div class="portlet-body">
							<h2>Guidelines for Rating</h2>
							<h4>A - Outstanding</h4> <h4>B - Very Good</h4>
							<h4>C - Good</h4>
							<h4>D - Fair</h4>
							<h4>E - Poor</h4>
							<div id="sample_editable_1_wrapper" class="dataTables_wrapper no-footer">
					        <div class="table-scrollable">
					        <table class="table table-striped table-hover table-bordered dataTable no-footer" id="sample_editable_1" role="grid" aria-describedby="sample_editable_1_info">
							<thead>
							<tr role="row">
								<th class="" tabindex="0" aria-controls="sample_editable_1" rowspan="1" colspan="1" aria-sort="ascending" aria-label="
									Learning Areas
								: activate to sort column ascending" style="width: 1000px;">
									 <center>Character Traits</center>
								</th>
								<th class=" " tabindex="0" aria-controls="sample_editable_1" rowspan="1" colspan="1" aria-label="
									 1
								: activate to sort column ascending" style="width: 100px;"><center>1</center>
									 
								</th>
								<th class=" " tabindex="0" aria-controls="sample_editable_1" rowspan="1" colspan="1" aria-label="
									 1
								: activate to sort column ascending" style="width: 100px;"><center>2</center>
									 
								</th>
								<th class=" " tabindex="0" aria-controls="sample_editable_1" rowspan="1" colspan="1" aria-label="
									 1
								: activate to sort column ascending" style="width: 100px;"><center>3</center>
									 
								</th>
								<th class=" " tabindex="0" aria-controls="sample_editable_1" rowspan="1" colspan="1" aria-label="
									 1
								: activate to sort column ascending" style="width: 100px;"><center>4</center>
									 
								</th>
								
								
								
								<th class=" " tabindex="0" aria-controls="sample_editable_1" rowspan="1" colspan="1" aria-label="
									 Edit
								: activate to sort column ascending" style="width: 91px;">
									 <center> Edit</center>
								</th>
								</tr>
							</thead>
							<tbody>
				
						
						
							<tr role="row" class="odd">
								<td class=" _1">
									1. Honesty
								</td>
								<td>
									
								</td>
								<td>
									
								</td>
								<td>
									
								</td>
								<td>
									
								</td>
								<td>
									<a class="edit" href="javascript:;">
									Edit </a>
								</td>
							</tr>
							<tr role="row" class="odd">
								<td class=" _1">
									2. Courtesy
								</td>
								<td>
									
								</td>
								<td>
									
								</td>
								<td>
									
								</td>
								<td>
									
								</td>
								<td>
									<a class="edit" href="javascript:;">
									Edit </a>
								</td>
							</tr>
							<tr role="row" class="odd">
								<td class=" _1">
									3. Helpfulness and Cooperation
								</td>
								<td>
									
								</td>
								<td>
									
								</td>
								<td>
									
								</td>
								<td>
									
								</td>
								<td>
									<a class="edit" href="javascript:;">
									Edit </a>
								</td>
							</tr>
							<tr role="row" class="odd">
								<td class=" _1">
									Resourcefulness and Creativity
								</td>
								<td>
									
								</td>
								<td>
									
								</td>
								<td>
									
								</td>
								<td>
									
								</td>
								<td>
									<a class="edit" href="javascript:;">
									Edit </a>
								</td>
							</tr>
							<tr role="row" class="odd">
								<td class=" _1">
									5. Consideration for Others
								</td>
								<td>
									
								</td>
								<td>
									
								</td>
								<td>
									
								</td>
								<td>
									
								</td>
								<td>
									<a class="edit" href="javascript:;">
									Edit </a>
								</td>
							</tr>
							<tr role="row" class="odd">
								<td class=" _1">
									6. Sportmanship
								</td>
								<td>
									
								</td>
								<td>
									
								</td>
								<td>
									
								</td>
								<td>
									
								</td>
								<td>
									<a class="edit" href="javascript:;">
									Edit </a>
								</td>
							</tr>
							<tr role="row" class="odd">
								<td class=" _1">
									7. Obedience
								</td>
								<td>
									
								</td>
								<td>
									
								</td>
								<td>
									
								</td>
								<td>
									
								</td>
								<td>
									<a class="edit" href="javascript:;">
									Edit </a>
								</td>
							</tr>
							<tr role="row" class="odd">
								<td class=" _1">
									8. Self-Reliance
								</td>
								<td>
									
								</td>
								<td>
									
								</td>
								<td>
									
								</td>
								<td>
									
								</td>
								<td>
									<a class="edit" href="javascript:;">
									Edit </a>
								</td>
							</tr>
							<tr role="row" class="odd">
								<td class=" _1">
									9. Industry
								</td>
								<td>
									
								</td>
								<td>
									
								</td>
								<td>
									
								</td>
								<td>
									
								</td>
								<td>
									<a class="edit" href="javascript:;">
									Edit </a>
								</td>
							</tr>
							<tr role="row" class="odd">
								<td class=" _1">
									10. Cleanliness and Orderliness
								</td>
								<td>
									
								</td>
								<td>
									
								</td>
								<td>
									
								</td>
								<td>
									
								</td>
								<td>
									<a class="edit" href="javascript:;">
									Edit </a>
								</td>
							</tr>
							<tr role="row" class="odd">
								<td class=" _1">
									11. Promptness & Punctuality
								</td>
								<td>
									
								</td>
								<td>
									
								</td>
								<td>
									
								</td>
								<td>
									
								</td>
								<td>
									<a class="edit" href="javascript:;">
									Edit </a>
								</td>
							</tr>
							<tr role="row" class="odd">
								<td class=" _1">
									12. Sense of Responsibility
								</td>
								<td>
									
								</td>
								<td>
									
								</td>
								<td>
									
								</td>
								<td>
									
								</td>
								<td>
									<a class="edit" href="javascript:;">
									Edit </a>
								</td>
							</tr>
							<tr role="row" class="odd">
								<td class=" _1">
									13. Love of God
								</td>
								<td>
									
								</td>
								<td>
									
								</td>
								<td>
									
								</td>
								<td>
									
								</td>
								<td>
									<a class="edit" href="javascript:;">
									Edit </a>
								</td>
							</tr>
							<tr role="row" class="odd">
								<td class=" _1">
									14. Patriotism & Love of Country
								</td>
								<td>
									
								</td>
								<td>
									
								</td>
								<td>
									
								</td>
								<td>
									
								</td>
								<td>
									<a class="edit" href="javascript:;">
									Edit </a>
								</td>
							</tr>
							<tr role="row" class="odd">
								<td class=" _1">
									<STRONG>Average</STRONG>
								</td>
								<td>
									
								</td>
								<td>
									
								</td>
								<td>
									
								</td>
								<td>
									
								</td>
								<td>
									<a class="edit" href="javascript:;">
									Edit </a>
								</td>
							</tr>
			
							
					
				
				
				
							</tbody>
							</table>
							</div>
							</div>
					</div>
					</div>
					</div>








<?php $this->load->view('admin/components/page_tail'); ?>